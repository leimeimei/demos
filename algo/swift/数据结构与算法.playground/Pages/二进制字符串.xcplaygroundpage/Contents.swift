...
func addBinary(a: NSString, b: NSString) -> String {
    ...
    let la = a.length
    let lb = b.length
    if la == 0 { return b as String }
    if lb == 0 { return a as String }
    ...
    for i in 0..<lmax {
        ...
        //二进制位相同位置2个数字同为0或者同为1，相加为0，不同则此位置肯定为1
        let s = (ca == cb ? "0" : "1")
        
        //相同位置的2个数字相加后，和本位的进位相加，相同为0，不同为1
        let sj = (s == add ? "0" : "1")
        //判断是否有进位
        ...
        res += sj  //当前位置2个数（0或者1）的相加后的结果
    }
    if add == "1" { //如果最后有进位，则要加上1位，值为1
        res += add
    }
    return String(res.reversed())  //字符串逆序得到正确的二进制数
}

//print(addBinary(a: "1001", b: "101"))
